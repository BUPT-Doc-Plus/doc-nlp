...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        for arg_clean in ('config_path', 'type_vocab_size', 'relax_projection', 'task_idx', 'max_position_embeddings', 'fp32_embedding', 'ffn_type', 'label_smoothing'):
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                num_tokens_a=None, num_tokens_b=None):
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
        ...
        self.mode = mode
        if self.mode not in ["s2s", "l2r", "both"]:
            raise ValueError("Invalid mode for decoder: %s" % self.mode)

    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx)
        if self.mode == "both":
            raise NotImplementedError(
                "sync decoder not implemented for greedy decoder")
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None):
        if self.mode == "both":
            return self.beam_search_sync(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx)
        ...
        while next_pos < output_length:
            ...
            if len(total_scores) == 0:
                ...
                back_ptrs = torch.div(k_ids, K).int()
            ...
            def select_beam_items(x, ids):
                ...
                y = torch.gather(x, 1, ids.to(torch.int64))
        ...
        # back tracking
        traces = {'pred_seq': [], 'scores': [], 'wids': [], 'ptrs': [], 'pred_score': []}
        for b in range(batch_size):
            # [(beam,)]
            ...
            if frame_id == -1:
                ...
                traces['pred_score'].append([max_score])
            else:
                ...
                traces['pred_score'].append([max_score] * len(seq))
        ...
        for k in ('pred_seq', 'scores', 'wids', 'ptrs', 'pred_score'):
            ts_list = traces[k]
            if not isinstance(ts_list[0], torch.Tensor):
                dt = torch.float if k == 'scores' or k == 'pred_score' else torch.long
    ...
    def beam_search_sync(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None):
        input_shape = list(input_ids.size())
        if input_shape[0] % 2 != 0:
            raise ValueError(
                "invalid batch size for sync decoder, must be an even number, but get %d" % input_shape[0])
        ...
        while next_pos < output_length:
            ...
            prediction_scores, _ = self.cls(
                last_hidden, None, task_idx=task_idx)
            prb_scores = torch.nn.functional.softmax(prediction_scores, dim=-1)
            x_dim, _, vcb_size = list(prb_scores.size())
            prb_scores = torch.reshape(
                prb_scores, [x_dim // 2, 2, 1, vcb_size])
            prb_scores = torch.mean(prb_scores, dim=1) + 1e-20
            log_scores = torch.log(prb_scores)
            if forbid_word_mask is not None:
                log_scores += (forbid_word_mask * -10000.0)
            kk_scores, kk_ids = torch.topk(log_scores, k=K)
            if len(total_scores) == 0:
                k_ids = torch.reshape(kk_ids, [batch_size, K])
                back_ptrs = torch.zeros(batch_size, K, dtype=torch.int)
                k_scores = torch.reshape(kk_scores, [batch_size, K])
            else:
                last_eos = torch.reshape(
                    beam_masks[-1], [batch_size * K, 1, 1])
            ...
            def first_expand_sync(x):
                input_shape = list(x.size())
                expanded_shape = [input_shape[0] // 2, 1, 2] + input_shape[1:]
                x = torch.reshape(x, expanded_shape)
                repeat_count = [1, K] + [1] * (len(expanded_shape) - 2)
                x = x.repeat(*repeat_count)
                x = torch.reshape(x, [input_shape[0] * K] + input_shape[1:])
                return x

            def select_beam_items_sync(x, ids):
                id_shape = list(ids.size())
                assert len(id_shape) == 2
                ids = torch.reshape(ids, id_shape + [1])
                ids = ids.repeat(*[1, 1, 2])
                id_shape = list(ids.size())
                id_rank = len(id_shape)
                x_shape = list(x.size())
                x = torch.reshape(x, [batch_size, K, 2] + x_shape[1:])
                x_rank = len(x_shape) + 2
                assert x_rank >= 3
                if id_rank < x_rank:
                    ids = torch.reshape(
                        ids, id_shape + [1] * (x_rank - id_rank))
            ...
            if prev_embedding is None:
                prev_embedding = first_expand_sync(new_embedding[:, :-1, :])
            else:
                prev_embedding = torch.cat(
                    (prev_embedding, new_embedding[:, :-1, :]), dim=1)
                prev_embedding = select_beam_items_sync(
                    prev_embedding, back_ptrs)
            if prev_encoded_layers is None:
                prev_encoded_layers = [first_expand_sync(
                    x[:, :-1, :]) for x in new_encoded_layers]
            else:
                prev_encoded_layers = [torch.cat((x[0], x[1][:, :-1, :]), dim=1)
                                       for x in zip(prev_encoded_layers, new_encoded_layers)]
                prev_encoded_layers = [select_beam_items_sync(
                    x, back_ptrs) for x in prev_encoded_layers]

            curr_ids = torch.reshape(k_ids, [batch_size * K, 1])
            curr_ids = curr_ids.repeat(*[1, 2])
            curr_ids = torch.reshape(curr_ids, [batch_size * K * 2, 1])

            if is_first:
                token_type_ids = first_expand_sync(token_type_ids)
                position_ids = first_expand_sync(position_ids)
                attention_mask = first_expand_sync(attention_mask)
                mask_ids = first_expand_sync(mask_ids)
        ...
        for b in range(batch_size):
            ...
            ptrs = [x[b] for x in step_back_ptrs]
            score_traces.append(
                {"scores": scores, "wids": wids_list, "ptrs": ptrs})
            # first we need to find the eos frame where all symbols are eos
            # any frames after the eos frame are invalid
            last_frame_id = len(scores) - 1
            for i, wids in enumerate(wids_list):
        ...
        return output_sequences, score_traces
