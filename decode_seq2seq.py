...
def main():
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        if args.n_samples != -1:
            input_lines = input_lines[:args.n_samples]
        logger.info("Finished loading {} samples".format(len(input_lines)))
            ...
            while next_i < len(input_lines):
                ...
                with torch.no_grad():
                    ...
                    for i in range(len(buf)):
                        ...
                        output_sequence = ' '.join(detokenize(output_tokens))   #' '.join(detokenize(output_tokens)) + "@@~~@@" + str(w_score)
