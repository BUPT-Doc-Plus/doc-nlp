...
class StanfordCoreNLP:
    def __init__(self, path_or_host,
                 ...
                 logging_level=logging.WARNING, max_retries=10):
        ...
        while sock.connect_ex((host_name, self.port)):
            if trial > max_retries:
                raise ValueError(
                    'Connection Error. Corenlp server is not available. Try Again!')
    ...
    def sent_split(self, document):
        r_dict = self._request(self.url, 'ssplit', document)
        sent_dicts = r_dict['sentences']
        sentences = []
